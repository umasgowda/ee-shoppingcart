...
public class ShoppingCartFirstStep {
    ...
    public String getShoppingCartTotalPrice() {
        final BigDecimal totalPrice = shoppingCartProducts.stream().map(product -> BigDecimal.valueOf(product.getQuantity()).multiply(product.getUnitPrice())).reduce(BigDecimal.ZERO, BigDecimal::add);
